...
def mult_basic_test(dut):
    ...
    for val in gen_sin():
        ...
        if multiplier_model(1, int(2**15 * val)) != translate_res(dut.z) :
...
def mult_randomised_test(dut):
    ...
    for i in range(10):
        ...
        dut.y = B
        yield Timer(2)
        if A*B != multiplier_model(A, B):
        #if int(dut.z) != multiplier_model(A, B):
            raise TestFailure("Randomised test failed with: %s * %s = %s" %(int(dut.x), int(dut.y), int(dut.z)))
        else:  # these last two lines are not strictly necessary
            dut._log.info("Ok!")